...
function getAppPath(): string {
  return process.env.PLAYWRIGHT_ELECTRON_APP_PATH ||
...
test.describe('AtlasVibe UI Tests - Docker', () => {
  ...
  test('Sidebar navigation is functional', async () => {
    ...
    for (const item of navItems) {
      const navButton = await page.locator(`button:has-text("${item}"), a:has-text("${item}")`).first();
    ...
    const canvas = await page.locator('canvas, [data-testid="flow-canvas"], .react-flow').first();
    await expect(canvas).toBeVisible();
  });

  test('Block palette displays categories', async () => {
    // Navigate to blocks section
    const blocksButton = await page.locator('button:has-text("Blocks"), a:has-text("Blocks")').first();
  ...
  test('Drag and drop block to canvas', async () => {
    ...
    if (blockExists) {
      ...
      const droppedBlock = await page.locator('.react-flow__node, [data-testid="flow-node"]').first();
