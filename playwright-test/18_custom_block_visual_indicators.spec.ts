...
test.describe("Custom block visual indicators", () => {
  ...
  test("Should display custom blocks with distinct visual styling", async () => {
    ...
    // Get both blocks
    const blueprintBlock = window
      .locator('[data-testid^="block-"]', { hasText: "RAND" })
      .first();
    const customBlock = window
      .locator('[data-testid^="block-"]', { hasText: "CUSTOM_STYLED_BLOCK" })
      .first();
  ...
  test("Should have different context menu for custom blocks", async () => {
    ...
    const blueprintMenuItems = await blueprintMenu
      .locator("button")
      .allTextContents();
    ...
    const customMenuItems = await customMenu
      .locator("button")
      .allTextContents();
  ...
  test("Should highlight custom block when editing parameters", async () => {
    ...
    await expect(window.locator('[data-testid="control-panel"]')).toBeVisible();
    ...
    if (await paramControl.isVisible({ timeout: 1000 })) {
      ...
      expect(
        focusedStyle.boxShadow !== "none" || focusedStyle.outline !== "none",
      ).toBeTruthy();
    }
  });
...
`,
    ...
    const errorBlock = window
      .locator('[data-testid^="block-"]', { hasText: "ERROR_TEST_BLOCK" })
      .first();
    if (await errorBlock.isVisible({ timeout: 2000 })) {
      // Check for error styling
      const errorClasses = await errorBlock.getAttribute("class");
      const hasErrorClass =
        errorClasses?.includes("error") || errorClasses?.includes("invalid");
  ...
  test("Should update visual indicators when custom block is modified", async () => {
    ...
    if (await editOption.isVisible({ timeout: 1000 })) {
      ...
      const modifiedBadge = customBlock.locator(
  ...
  test("Should group custom blocks visually in the sidebar", async () => {
    ...
    if (await customTab.isVisible({ timeout: 1000 })) {
      ...
      for (let i = 0; i < blockCount; i++) {
        const blockType = await blockButtons
          .nth(i)
