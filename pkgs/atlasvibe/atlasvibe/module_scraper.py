...
class AtlasvibeWrapper:
    ...
    CUSTOM_DOC_ADDITION += """The parameters of the function in this Atlasvibe wrapper are given below."""
    CUSTOM_DOC_ADDITION += "\n\t" + "-." * 36 + "\n"
    ...
    def __init__(self, func, parameters, module, argument_names):
        ...
        try:
            ...
            self.parameters = {}
        self.data = "from atlasvibe import DataContainer, atlasvibe\n"
        self.data += f"import {module.__name__}\n\n"
        self.data += f"@atlasvibe\ndef {self.name.upper()}(dc, params):\n\t"
            ...
            for param in self.parameters.keys():
                if param not in self.FORBIDDEN_OPTIONAL_ARGS:
                    ...
                    if dtype == "str":
                        ...
                        dtype = "boolean"
                    parameter_string += f"\t{param}: \n" + "\t\t\t\ttype: " + dtype + "\n\t\t\t\tdefault: " + ("" if def_val == "None" else def_val) + " \n\t\t"
            if parameter_string != "":
                ...
                if func_is_valid:
                    ...
                    if fw.manifest != "" and fw.data != "" and "NoneType" not in fw.data:
                        try:
                            ...
                            with open(this_nodes_directory / f"{fw.name.upper()}.py", "w") as fh:
                                fh.write(fw.data)
