...
class JobResultBuilder:
    ...
    def flow_to_nodes(self, nodes: list[str]):
        if nodes.__len__() > 0:
            self._add_instructions({ATLASVIBE_INSTRUCTION.FLOW_TO_NODES: nodes})
        return self
    ...
    def build(self):
        ...
        if self.instructions:
            result = {
                ...
                **self.instructions,
                # instruction to fetch_input method in atlasvibe.py
                ATLASVIBE_INSTRUCTION.RESULT_FIELD: "data",
                "data": result,
            }
        return result
