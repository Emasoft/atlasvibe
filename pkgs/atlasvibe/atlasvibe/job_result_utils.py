...
def get_next_nodes(result: dict[str, Any] | None) -> list[str]:
    if result is None:
        return []
    return cast(list[str], result.get(ATLASVIBE_INSTRUCTION.FLOW_TO_NODES, []))
...
def get_dc_from_result(
    ...
    if isinstance(result, DataContainer):
        return result
    if result.get(ATLASVIBE_INSTRUCTION.RESULT_FIELD):
        return result[result[ATLASVIBE_INSTRUCTION.RESULT_FIELD]]
    return result["data"]
...
def get_frontend_res_obj_from_result(
    ...
    if result.get(ATLASVIBE_INSTRUCTION.RESULT_FIELD):
        data = result[result[ATLASVIBE_INSTRUCTION.RESULT_FIELD]]
        if not data:
            return result
