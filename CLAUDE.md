...
- _CRITICAL_: Never use GREP! Use RIPGREP instead!
- _CRITICAL_: Never use pip. Use `uv pip <commands>` instead. Consider pip deprecated in favor of uv pip.
- _CRITICAL_: Never spawn multiple subagents that need to use git at the same time. It can cause conflicting git operations that lead to repo corruption.
...
- Run mypy using this command: `COLUMNS=400 uv run mypy --strict --show-error-context --pretty --install-types --no-color-output --show-error-codes --show-error-code-links --no-error-summary --follow-imports=normal <files to test or pattern...>`
...
- After committing and pushing the project to github, always check if the push passed the github actions and checks. Wait few seconds, according to the average time needed for the lint and tests to run, and retrieve the results.
...
- **Beautiful Test Report**: Use `uv run python beautiful_test_report.py` for formatted test output
...
1. **Always use uv for Python operations** - Never use pip, venv, or virtualenv directly
2. **Commit uv.lock** - Ensures reproducible builds across machines
3. **Use .python-version** - Pin Python version for consistency
4. **Leverage workspaces** - For multi-package projects like AtlasVibe
...
source .venv/bin/activate  # Activate the venv on Linux/macOS
.venv_windows\Scripts\activate     # Activate the venv on Windows
uv init <...options...>    # Init the project (use --app for applications or --lib for libraries)
uv add -r requirements.txt # Add the requirements to the pyproject.toml for uv to manage
uv pip install -r requirements.txt
uv pip install -e .        # Setup the venv for development
...
uv run python main.py        # Run backend
uv run pnpm run dev                 # Run frontend (separate terminal)
```
...
- **Import Path Updates**: Changed from `atlasvibe` to `pkgs.atlasvibe.atlasvibe`
...
- **Tasks**:
  - Runs actionlint to check workflow syntax
...
- **Jobs**:
  - **block-completeness**: Verifies all blocks have required metadata files (runs `fjblock.py sync`)
  - **block-code-format**: Checks Python formatting with Ruff
  - **block-code-lint**: Runs Ruff linter on all code
  - **pytest**: Runs unit tests on Ubuntu, macOS, and Windows (excludes slow tests)
- **Key Commands**:
...
- **Tasks**:
  - Installs dependencies with uv and pnpm
...
- **Jobs**:
  ...
  - **ts-code-style**: TypeScript/JavaScript checks (ESLint, TSC, Prettier)
  - **python-tests**: Runs pytest with coverage reporting
- **Key Features**:
  - Uses codecov for test coverage reporting
...
- **Tasks**:
  - Builds platform-specific Electron packages
  - Sets up Xvfb for Linux display
  - Runs Playwright E2E tests
  - Uploads test results as artifacts
- **Special Requirements**: Linux requires Xvfb for headless testing
...
- Removed obsolete test_pr.yml workflow
...
- **Beautiful test reporting**: New `beautiful_test_report.py` generates formatted tables with:
  - Unicode borders and color coding
  - Test descriptions from docstrings
  - Snail emoji 🐌 for slow tests
  - Proper alignment accounting for emoji width
