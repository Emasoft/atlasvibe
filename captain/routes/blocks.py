    ...
    try:
        ...
        async with managed_operation(
            ...
            try:
                ...
                logger.info(
                    f"[{request_id}] Queued code update for block '{block_name}' - transaction {transaction_id}"
                )
        ...
        logger.info(
            f"Code update for block '{block_name}' (ID: {block_id}) - transaction {transaction_id} - status: {response['status']}"
        )
