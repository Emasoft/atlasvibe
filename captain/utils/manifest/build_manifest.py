...
def populate_inputs(
    ...
    # Case 5: Some kind of list
        ...
        if param_type not in ALLOWED_PARAM_TYPES:
            raise TypeError(f"Parameter types must be one of {ALLOWED_PARAM_TYPES} or special types like NodeReference,got {param_type}")
    ...
    def populate(name: str, param: Parameter):
        ...
        if is_union(param_type):
            ...
            if param_type not in ALLOWED_PARAM_TYPES and param_type != Array:
                raise TypeError(f"Parameter types must be one of {ALLOWED_PARAM_TYPES} or Arraygot {param_type}")
        ...
        for attr, value in dict(return_type.__annotations__).items():
            if is_special_node:
                ...
                if not issubclass(value, DataContainer):
                    raise TypeError(f"Return type must be a DataContainer or a typing.TypedDictconsisting of only DataContainers as fields, got {return_type}")
