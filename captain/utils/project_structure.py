...
def validate_block_name(name: str) -> None:
    ...
    if not re.match(r"^[A-Za-z][A-Za-z0-9_]*$", name):
        raise ProjectStructureError(
            "Block name must start with a letter and contain only letters, numbers, and underscores"
        )
...
def update_block_function_name(py_file: Path, old_name: str, new_name: str) -> None:
...
def update_block_metadata(block_dir: Path, old_name: str, new_name: str) -> None:
