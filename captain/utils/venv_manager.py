...
class VenvManager:
    ...
    def regenerate(self, dependencies: Optional[List[str]] = None,
        ...
        try:
            ...
            critical_failures = [c for c in pre_checks 
                               if c.status == CheckStatus.ERROR and 
                               c.name in ["python_version", "disk_space", "permissions", "uv_availability"]]
            ...
            critical_post_failures = [c for c in post_checks 
                                    if c.status == CheckStatus.ERROR and
                                    c.name in ["import_test", "block_execution"]]
        ...
        try:
            ...
            if version_info < self.MIN_PYTHON_VERSION:
                ...
                return CheckResult(
                    ...
                    details={"current": version_str, "maximum": f"{self.MAX_PYTHON_VERSION[0]}.{self.MAX_PYTHON_VERSION[1]}"},
                    duration=time.time() - start
                )
            else:
                return CheckResult(
        ...
        try:
            ...
            if available_mb < self.MIN_DISK_SPACE_MB:
                return CheckResult(
                    ...
                    details={"available_mb": available_mb, "required_mb": self.MIN_DISK_SPACE_MB},
                    duration=time.time() - start,
...
def regenerate_venv(block_path: str, dependencies: Optional[List[str]] = None,
                   python_version: Optional[str] = None) -> Dict[str, Any]:
