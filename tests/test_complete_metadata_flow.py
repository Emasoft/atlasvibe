...
""")
            ...
            with (
                patch(
                    "captain.utils.blocks_path.get_blocks_path",
                    return_value=str(Path(blueprint_base) / "blocks"),
                ),
                patch(
                    "captain.routes.blocks.find_blueprint_path",
                    return_value=blueprint_dir,
                ),
            ):
                # Step 2: Create custom block via API
                response = test_client.post(
                    "/blocks/create-custom/",
                    json={
...
"""
    ...
    async def test_file_watcher_websocket_flow(self, test_project):
        ...
        with (
            patch(
                "captain.services.consumer.blocks_watcher.ConnectionManager.get_instance",
                return_value=mock_ws,
            ),
            patch(
                "captain.services.consumer.blocks_watcher.get_blocks_path",
                return_value=str(test_project["blocks_dir"]),
            ),
        ):
