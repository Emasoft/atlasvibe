...
from mass_find_replace import main_flow, MAIN_TRANSACTION_FILE_NAME
...
def test_standard_run(temp_test_dir: Path, default_map_file: Path, ignore_symlinks: bool):
    ...
    elif (temp_test_dir / "binary_atlasvibe_file.bin").exists(): # If the renamed binary file exists
    ...
    rtf_content_tx = next((tx for tx in transactions if "test_atlasvibe.rtf" in tx["PATH"] and tx["TYPE"] == TransactionType.FILE_CONTENT_LINE.value), None) # Path after rename
    if not rtf_content_tx: # Check original path if rename didn't happen or was separate tx
        rtf_content_tx = next((tx for tx in transactions if "test_flojoy.rtf" == tx.get("PATH") and tx["TYPE"] == TransactionType.FILE_CONTENT_LINE.value), None)
    assert rtf_content_tx is not None, "RTF file content transaction should have been planned"
    assert rtf_content_tx["STATUS"] == TransactionStatus.SKIPPED.value, "RTF content transaction should be SKIPPED"
...
@pytest.mark.parametrize("filename, content_bytes, is_text_expected, contains_flojoy_bytes", [
    ("text_file.txt", b"This is a plain text file with flojoy.", True, True),
    ("utf16_file.txt", "UTF-16 text with flojoy".encode('utf-16'), True, True),
    ("binary_file.bin", b"\x00\x01\x02flojoy_data\x03\x04\xDE\xAD\xBE\xEF", False, True),
    ("image.jpg", b"\xFF\xD8\xFF\xE0\x00\x10JFIF\x00\x01flojoy_marker", False, True),
    ("control_char_heavy.txt", b"text" + b"\x01\x02\x03\x04\x05" * 200 + b"flojoy", False, True), # >5% control chars
    ("mostly_nulls.dat", b"\x00" * 500 + b"flojoy" + b"\x00" * 500, False, True), # >5% nulls
    ("valid_utf8.txt", "Valid UTF-8 string with flojoy and éàçüö.".encode('utf-8'), True, True),
    ("corrupt_utf8.txt", b"Corrupt UTF-8 with flojoy \xff\xfe some more.", False, True),
    ("empty_file.txt", b"", True, False),
    ("rtf_file.rtf", b"{\\rtf1\\ansi This is flojoy rtf text.}", True, True), # RTF is text-like after extraction
    ("svg_file.svg", b"<svg><text>flojoy</text></svg>", True, True),
    ("no_match_binary.dat", b"\xDE\xAD\xBE\xEF\xCA\xFE\xBA\xBE", False, False),
    ("no_match_text.txt", b"Simple text file.", True, False),
])
def test_binary_detection_and_processing(temp_test_dir: Path, default_map_file: Path, filename, content_bytes, is_text_expected, contains_flojoy_bytes):
    from file_system_operations import is_likely_binary_file # Test this directly
    
    file_path = temp_test_dir / filename
    file_path.write_bytes(content_bytes)

    detected_as_binary = is_likely_binary_file(file_path)
    assert not detected_as_binary if is_text_expected else detected_as_binary, \
        f"File {filename} binary detection mismatch. Expected text: {is_text_expected}, Detected binary: {detected_as_binary}"
    ...
    if is_text_expected:
        if contains_flojoy_bytes:
            assert content_tx_found, f"Expected content transaction for text file {filename}."
            if file_path.suffix.lower() != '.rtf': # RTF content modification is skipped
                # Read back with a common encoding for check, assuming replacement makes it valid UTF-8
        ...
        assert not binary_log_has_match, f"Text file {filename} should not have matches in binary log."
...
@pytest.mark.slow 
def test_timeout_behavior(temp_test_dir: Path, default_map_file: Path):
    # This test is conceptual for now, as reliably mocking file locks/hangs is complex.
    # It verifies the timeout option is accepted and the flow completes.
    create_test_environment_content(temp_test_dir)
    # To truly test timeout, one of the _execute functions would need to be mocked
    # to simulate a long delay or repeated retryable errors.
    run_main_flow_for_test(temp_test_dir, default_map_file, timeout_minutes=1) # 1 minute timeout (actually 1 sec in test)
    transactions = load_transactions(temp_test_dir / MAIN_TRANSACTION_FILE_NAME)
    assert transactions is not None, "Transaction file should exist."
    # A more robust test would check if any transaction is FAILED with "Global timeout reached".

def test_empty_directory_handling(temp_test_dir: Path, default_map_file: Path, caplog):
    # temp_test_dir is empty by default from fixture.
    # We expect main_flow to log an info message and exit early.
    # caplog fixture captures log messages.
    import logging
    caplog.set_level(logging.INFO)

    run_main_flow_for_test(temp_test_dir, default_map_file) # Should exit early

    assert any("Target directory" in record.message and "is empty" in record.message for record in caplog.records)
    txn_file = temp_test_dir / MAIN_TRANSACTION_FILE_NAME
    assert not txn_file.exists() or txn_file.stat().st_size == 0
