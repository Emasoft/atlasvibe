...
def mock_blueprint_base_path(tmp_path: Path) -> Path:
    ...
    (matrix_view_bp / (constants.BLUEPRINT_MATRIX_VIEW + constants.PYTHON_FILE_EXT)).write_text(f"# Original {constants.BLUEPRINT_MATRIX_VIEW}\n@atlasvibe_node\ndef {constants.BLUEPRINT_MATRIX_VIEW}(data):\n  return data")
    ...
    add_bp.mkdir()
    (add_bp / (constants.BLUEPRINT_ADD + constants.PYTHON_FILE_EXT)).write_text(f"@atlasvibe_node\ndef {constants.BLUEPRINT_ADD}(a, b):\n  return a + b")
    ...
    constant_bp.mkdir()
    (constant_bp / (constants.BLUEPRINT_CONSTANT + constants.PYTHON_FILE_EXT)).write_text(f"@atlasvibe_node\ndef {constants.BLUEPRINT_CONSTANT}():\n  return 123")
    ...
    another_bp.mkdir()
    (another_bp / ("ANOTHER_NODE" + constants.PYTHON_FILE_EXT)).write_text("@atlasvibe_node\ndef ANOTHER_NODE():\n  return 'another'")
...
def test_rename_block_handles_python_symbol_collision_simplified(block_service_instance: MockBlockService, temp_project_dir: Path):
    ...
    os.makedirs(
        os.path.join(project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, problematic_name),
        exist_ok=True,
    )
