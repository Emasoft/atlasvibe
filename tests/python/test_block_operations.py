...
def mock_blueprint_base_path(tmp_path: Path) -> Path:
    ...
    (
        matrix_view_bp / (constants.BLUEPRINT_MATRIX_VIEW + constants.PYTHON_FILE_EXT)
    ).write_text(
        f"# Original {constants.BLUEPRINT_MATRIX_VIEW}\n@atlasvibe_node\ndef {constants.BLUEPRINT_MATRIX_VIEW}(data):\n  return data"
    )
    (matrix_view_bp / constants.METADATA_APP_JSON).write_text(
        f'{{"name": "{constants.BLUEPRINT_MATRIX_VIEW}", "key": "{constants.BLUEPRINT_MATRIX_VIEW}"}}'
    )
    ...
    (add_bp / constants.METADATA_APP_JSON).write_text(
        f'{{"name": "{constants.BLUEPRINT_ADD}", "key": "{constants.BLUEPRINT_ADD}"}}'
    )
    ...
    constant_bp.mkdir()
    (
        constant_bp / (constants.BLUEPRINT_CONSTANT + constants.PYTHON_FILE_EXT)
    ).write_text(
        f"@atlasvibe_node\ndef {constants.BLUEPRINT_CONSTANT}():\n  return 123"
    )
    (constant_bp / constants.METADATA_APP_JSON).write_text(
        f'{{"name": "{constants.BLUEPRINT_CONSTANT}", "key": "{constants.BLUEPRINT_CONSTANT}"}}'
    )
    ...
    (another_bp / constants.METADATA_APP_JSON).write_text(
        '{"name": "ANOTHER_NODE", "key": "ANOTHER_NODE"}'
    )
...
def test_add_block_creates_custom_block_folder_and_files(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
    """
    ...
    created_block_info = block_service_instance.add_block_to_project(
        project_path_str, blueprint_key
    )

    expected_custom_block_name = f"{constants.BLUEPRINT_MATRIX_VIEW}_1"
    assert created_block_info["name"] == expected_custom_block_name

    custom_block_folder = os.path.join(
        project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, expected_custom_block_name
    )
    assert os.path.isdir(custom_block_folder)

    expected_py_file = os.path.join(
        custom_block_folder, expected_custom_block_name + constants.PYTHON_FILE_EXT
    )
    ...
    created_block_info_2 = block_service_instance.add_block_to_project(
        project_path_str, blueprint_key
    )
    ...
    custom_block_folder_2 = os.path.join(
        project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, expected_custom_block_name_2
    )
    assert os.path.isdir(custom_block_folder_2)


def test_gui_distinction_and_default_naming_backend_support(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
    """
    ...
    info1 = block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_MATRIX_VIEW
    )
    ...
    info2 = block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_MATRIX_VIEW
    )
    ...
    info_add = block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_ADD
    )
...
def test_rename_block_updates_folder_file_and_function_name(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
    """
    ...
    """
    project_path_str = str(temp_project_dir)
    initial_block_info = block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_CONSTANT
    )
    old_name = initial_block_info["name"]
    new_name_base = "MyRenamedConstant"

    rename_info = block_service_instance.rename_custom_block(
        project_path_str, old_name, new_name_base
    )
    ...
    old_folder_path = os.path.join(
        project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, old_name
    )
    new_folder_path = os.path.join(
        project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, final_new_name
    )

    assert not os.path.exists(old_folder_path), (
        "Old block folder should not exist after rename."
    )
    assert os.path.isdir(new_folder_path), (
        "New block folder was not created or not found."
    )

    expected_py_file = os.path.join(
        new_folder_path, final_new_name + constants.PYTHON_FILE_EXT
    )
    assert os.path.exists(expected_py_file), (
        ".py file was not renamed or not found in new folder."
    )

    with open(expected_py_file, "r") as f:
        content = f.read()
        assert f"def {final_new_name}(" in content, (
            "Decorated function name was not updated in .py file."
        )
        assert f"def {old_name}(" not in content, (
            "Old function name should not exist in .py file."
        )
...
def test_rename_block_to_its_current_name(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
    ...
    rename_info = block_service_instance.rename_custom_block(
        project_path_str, current_name, current_name
    )

    assert rename_info["new_name"] == current_name
    original_folder_path = os.path.join(
        project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, current_name
    )
    ...
    py_file = os.path.join(
        original_folder_path, current_name + constants.PYTHON_FILE_EXT
    )
...
def test_rename_block_to_its_base_name_when_available(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
    ...
    initial_block_info_1 = block_service_instance.add_block_to_project(
        project_path_str, "ANOTHER_NODE"
    )  # Creates ANOTHER_NODE_1
    old_name_1 = initial_block_info_1["name"]  # ANOTHER_NODE_1
    new_base_name_1 = "ANOTHER_NODE"

    rename_info_1 = block_service_instance.rename_custom_block(
        project_path_str, old_name_1, new_base_name_1
    )
    ...
    new_folder_path_1 = os.path.join(
        project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, rename_info_1["new_name"]
    )
    assert os.path.isdir(new_folder_path_1)

    # Case 2: Renaming "CONSTANT_1" to "MyUniqueBase" (which is not a blueprint and available)
    initial_block_info_2 = block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_CONSTANT
    )  # Creates CONSTANT_1
    old_name_2 = initial_block_info_2["name"]  # CONSTANT_1
    new_available_base_name = "MyUniqueBase"

    rename_info_2 = block_service_instance.rename_custom_block(
        project_path_str, old_name_2, new_available_base_name
    )
    assert rename_info_2["new_name"] == new_available_base_name
    new_folder_path_2 = os.path.join(
        project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, new_available_base_name
    )
    assert os.path.isdir(new_folder_path_2)


def test_rename_block_handles_collision_with_blueprint_name(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
    """Test Req 5: Collision check - new name is a blueprint name."""
    project_path_str = str(temp_project_dir)
    initial_block = block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_CONSTANT
    )  # CONSTANT_1

    # Add MATRIX_VIEW_1 to make MATRIX_VIEW_1 taken
    block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_MATRIX_VIEW
    )

    rename_info = block_service_instance.rename_custom_block(
        project_path_str, initial_block["name"], constants.BLUEPRINT_MATRIX_VIEW
    )
    ...
    new_folder_path = os.path.join(
        project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, rename_info["new_name"]
    )
    assert os.path.isdir(new_folder_path)


def test_rename_block_handles_collision_with_existing_custom_block(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
    """Test Req 5: Collision check - new name matches another custom block."""
    project_path_str = str(temp_project_dir)
    # block1 will be CONSTANT_1
    block1 = block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_CONSTANT
    )
    # block2 will be ADD_1
    block2 = block_service_instance.add_block_to_project(
        project_path_str, constants.BLUEPRINT_ADD
    )

    # Try to rename block2 (ADD_1) to "CONSTANT_1" (which is block1's name)
    rename_info = block_service_instance.rename_custom_block(
        project_path_str, block2["name"], block1["name"]
    )
...
def test_rename_block_handles_python_symbol_collision_simplified(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
    """
    ...
    problematic_name = (
        "pass"  # Not a blueprint, assume not an existing custom block initially
    )
    # Simulate a collision by creating a folder with this problematic name
    os.makedirs(
        os.path.join(
            project_path_str, constants.CUSTOM_BLOCKS_DIR_NAME, problematic_name
        ),
        exist_ok=True,
    )

    rename_info = block_service_instance.rename_custom_block(
        project_path_str, initial_block["name"], problematic_name
    )
...
def test_add_block_with_target_name_collision(
    block_service_instance: MockBlockService, temp_project_dir: Path
):
