...
def mock_blueprint_dir(tmp_path: Path) -> Path:
    ...
    input_node_bp.mkdir()
    (
        input_node_bp / (constants.BLUEPRINT_INPUT_NODE + constants.PYTHON_FILE_EXT)
    ).write_text(f"@atlasvibe_node\ndef {constants.BLUEPRINT_INPUT_NODE}(): pass")
    (input_node_bp / constants.METADATA_APP_JSON).write_text(
        f'{{"name": "{constants.BLUEPRINT_INPUT_NODE}", "key": "{constants.BLUEPRINT_INPUT_NODE}"}}'
    )
    ...
    proc_node_bp.mkdir()
    (
        proc_node_bp / (constants.BLUEPRINT_PROCESSING_NODE + constants.PYTHON_FILE_EXT)
    ).write_text(
        f"@atlasvibe_node\ndef {constants.BLUEPRINT_PROCESSING_NODE}(input): pass"
    )
    (proc_node_bp / constants.METADATA_APP_JSON).write_text(
        f'{{"name": "{constants.BLUEPRINT_PROCESSING_NODE}", "key": "{constants.BLUEPRINT_PROCESSING_NODE}"}}'
    )
    ...
    (
        output_node_bp / (constants.BLUEPRINT_OUTPUT_NODE + constants.PYTHON_FILE_EXT)
    ).write_text(f"@atlasvibe_node\ndef {constants.BLUEPRINT_OUTPUT_NODE}(input): pass")
    (output_node_bp / constants.METADATA_APP_JSON).write_text(
        f'{{"name": "{constants.BLUEPRINT_OUTPUT_NODE}", "key": "{constants.BLUEPRINT_OUTPUT_NODE}"}}'
    )
