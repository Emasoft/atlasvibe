...
class MockProjectService:
    def create_new_project(self, project_name, base_path):
        ...
        custom_blocks_path = os.path.join(
            project_path, constants.CUSTOM_BLOCKS_DIR_NAME
        )
        os.makedirs(custom_blocks_path, exist_ok=True)
        return project_path

    def create_project_from_template(
        self, template_name, new_project_name, base_path, block_service_instance
    ):
        ...
        if template_name == "BasicTemplate":
            mock_template_blocks = [
                {
                    "blueprint_key": constants.BLUEPRINT_INPUT_NODE,
                    "id_in_template": "input1",
                },
                {
                    "blueprint_key": constants.BLUEPRINT_PROCESSING_NODE,
                    "id_in_template": "proc1",
                },
                {
                    "blueprint_key": constants.BLUEPRINT_OUTPUT_NODE,
                    "id_in_template": "out1",
                },
            ]
...
class MockBlockService:
    ...
    def _get_next_available_name_in_project(
        self, project_custom_blocks_path, base_name
    ):
        suffix = 1
        while True:
            potential_name = f"{base_name}_{suffix}"
            if not os.path.exists(
                os.path.join(project_custom_blocks_path, potential_name)
            ):
                return potential_name
            suffix += 1

    def add_block_to_project(
        self, project_path, blueprint_key, target_custom_block_name=None
    ):
        if blueprint_key not in self.available_blueprints:
            ...
            if blueprint_key not in self.available_blueprints:
                raise ValueError(
                    f"Blueprint '{blueprint_key}' not found in {self.blueprint_base_path}. Available: {list(self.available_blueprints.keys())}"
                )

        project_custom_blocks_path = os.path.join(
            str(project_path), constants.CUSTOM_BLOCKS_DIR_NAME
        )
        ...
        if target_custom_block_name:
            ...
            if os.path.exists(
                os.path.join(project_custom_blocks_path, target_custom_block_name)
            ):
                ...
                final_block_name = self._get_next_available_name_in_project(
                    project_custom_blocks_path, blueprint_key
                )
        ...
        custom_block_folder_path = os.path.join(
            project_custom_blocks_path, final_block_name
        )
        ...
        blueprint_py_path = os.path.join(
            self.available_blueprints[blueprint_key]["path"], original_py_filename
        )
        ...
        modified_content = blueprint_content.replace(
            f"def {blueprint_key}", f"def {final_block_name}"
        )
        ...
        app_json_path = os.path.join(
            custom_block_folder_path, constants.METADATA_APP_JSON
        )
        original_app_json_path = os.path.join(
            self.available_blueprints[blueprint_key]["path"],
            constants.METADATA_APP_JSON,
        )
        app_data_to_write = {
            "name": final_block_name,
            "key": final_block_name,
            "original_blueprint": blueprint_key,
        }
        ...
        blueprint_test_file_path = os.path.join(
            self.available_blueprints[blueprint_key]["path"], original_test_filename
        )
        if os.path.exists(blueprint_test_file_path):
            shutil.copy(
                blueprint_test_file_path,
                os.path.join(custom_block_folder_path, new_test_filename),
            )
    ...
    def rename_custom_block(
        self, project_path, old_custom_block_name, new_name_proposal
    ):
        project_custom_blocks_path = os.path.join(
            str(project_path), constants.CUSTOM_BLOCKS_DIR_NAME
        )
        old_block_folder_path = os.path.join(
            project_custom_blocks_path, old_custom_block_name
        )

        if not os.path.exists(old_block_folder_path):
            raise FileNotFoundError(
                f"Custom block '{old_custom_block_name}' not found in project at {old_block_folder_path}."
            )
        ...
        if new_name_proposal == old_custom_block_name:
            ...
            final_new_name = self._get_next_available_name_in_project(
                project_custom_blocks_path, new_name_proposal
            )
        elif os.path.exists(
            os.path.join(project_custom_blocks_path, new_name_proposal)
        ):
        ...
        if final_new_name == old_custom_block_name:
            return {
                "old_name": old_custom_block_name,
                "new_name": final_new_name,
                "path": old_block_folder_path,
            }
        ...
        old_py_filename_in_new_folder = (
            old_custom_block_name + constants.PYTHON_FILE_EXT
        )
        new_py_filename = final_new_name + constants.PYTHON_FILE_EXT

        path_to_old_py_in_new_folder = os.path.join(
            new_block_folder_path, old_py_filename_in_new_folder
        )
        path_to_new_py_in_new_folder = os.path.join(
            new_block_folder_path, new_py_filename
        )
        ...
        if os.path.exists(py_filepath):
            ...
            modified_content = content.replace(
                f"def {old_custom_block_name}", f"def {final_new_name}"
            )
        ...
        # Rename test file
        old_test_filename_in_new_folder = (
            old_custom_block_name + constants.TEST_FILE_SUFFIX
        )
        new_test_filename = final_new_name + constants.TEST_FILE_SUFFIX
        path_to_old_test_in_new_folder = os.path.join(
            new_block_folder_path, old_test_filename_in_new_folder
        )
        if os.path.exists(path_to_old_test_in_new_folder):
            os.rename(
                path_to_old_test_in_new_folder,
                os.path.join(new_block_folder_path, new_test_filename),
            )
