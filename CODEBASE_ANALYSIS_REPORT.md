...
```typescript
function apiCall<T>(endpoint: string, options?: RequestOptions): ResultAsync<T, HTTPError> {
  return fromPromise(captain.post(endpoint, options).json(), e => e as HTTPError);
}
```
