...
vi.mock("@/renderer/lib/block", () => ({
  ...
  createBlockLabel: vi.fn(
    (func: string, taken: RegExpMatchArray[]) => `${func} ${taken.length + 1}`,
  ),
...
describe("Node Factory", () => {
  describe("createNodeFromBlock", () => {
    const mockBlockDefinition: BlockDefinition = {
      ...
      inputs: [
        { name: "in", id: "in", type: "float", desc: null, multiple: false },
      ],
    ...
    it("should throw error for custom block without path", () => {
      ...
      expect(() => createNodeFromBlock(options)).toThrow(
        "Custom blocks must have a path",
      );
    });
